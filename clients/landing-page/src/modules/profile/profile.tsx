...
const Profile = () => {
  ...
  const { profileData, imageData, courseData, followingUsers } =
    useProfileData();

  console.log("Following Users", followingUsers);

  const { followedUser } = useMemo(() => {
    const followedUser = followingUsers.filter((v) =>
      v.followers.some((v) => v?.studentId == userData?.id)
    );
    return {
      followedUser,
    };
    ...
    <div className="container">
      <div className="w-full xl:flex-row flex-col flex lg:gap-5 gap-3 relative">
              ...
              <Tabs>
                ...
                <Tab label="Timeline">
                  <div className="flex xl:flex-row flex-col w-full items-start gap-5">
                    <TimelineLeftSideElements data={followedUser} />
        ...
        <Modal
          ...
          onOpenChange={followingOnOpenChange}
        >
          <FollowingModal data={followingUsers} />
        </Modal>
      </div>
