...
import ReplySingleComment from "./reply-single-comment";

type Props = {
  data?: CommentType;
};

const SingleComment = ({ data }: Props) => {
  // state
  ...
  const { isOpen: replyForm, onOpenChange: replyFormChange } = useDisclosure();

  const { isOpen: deleteModal, onOpenChange: deleteModalChange } =
    useDisclosure();

  return (
    <div className="w-full flex md:pl-6 pl-0 justify-between items-start gap-3 group">
