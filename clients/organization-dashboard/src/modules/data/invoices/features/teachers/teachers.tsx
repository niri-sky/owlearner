"use client";
// global package
import { Box } from "@mui/material";
import Image from "next/image";
import { HiDotsVertical } from "react-icons/hi";
import { FaRegFile } from "react-icons/fa";
import { IoIosSearch } from "react-icons/io";
import { DataGrid, GridColDef } from "@mui/x-data-grid";
import {
  Dropdown,
  DropdownTrigger,
  DropdownMenu,
  DropdownItem,
  Button,
  Input,
  Modal,
  ModalContent,
  ModalHeader,
  ModalBody,
  ModalFooter,
  useDisclosure,
} from "@nextui-org/react";
// custop files import
import { faker } from "@faker-js/faker";
import { SearchBar, useSearchBar } from "@/shared/hooks/use-search-bar";
import { formatDate } from "@/shared/utils";
import { AiOutlinePlus } from "react-icons/ai";
import { useQuery } from "@apollo/client";
import { PAYMENT_INVOICES_QUERY } from "@/graphql/queries";
import useUserData from "@/shared/hooks/useUserData";
import BankDetailsModal from "../withdrawal/BankDetailsModal";
import TeacherInvoiceActions from "./TeacherInvoiceActions";

type Props = {
  data: PaymentInvoiceType[];
};

const TeachersInvoice = ({ data }: Props) => {
  const columns: GridColDef<PaymentInvoiceType>[] = [
    { field: "id", headerName: "#", flex: 0.1 },
    {
      field: "avatar",
      headerName: "Name",
      flex: 0.9,
      minWidth: 220,
      renderCell: (params) => {
        return (
          <div className="flex items-center gap-3">
            <div className="w-[38px] h-[38px] rounded-full">
              <Image
                src={params.row.teacher.profile || "/user-1.jpg"}
                alt=""
                width={38}
                height={38}
                className="w-[38px] mx-[5px] h-[38px] rounded-full"
              />
            </div>
            <div className="flex flex-col">
              <div className="text-sm">{params.row.teacher.name}</div>
              <a
                href={`mailto:${params.row.teacher.email}`}
                className="text-[12px]"
              >
                {params.row.teacher.email}
              </a>
            </div>
          </div>
        );
      },
    },
    // {
    //   field: "organizations_name",
    //   headerName: "Organizations",
    //   flex: 0.6,
    //   minWidth: 100,
    // },
    {
      field: "amount",
      headerName: "Amount",
      flex: 0.3,
      minWidth: 100,

      renderCell: (params: any) => {
        return (
          <>
            <div className="text-center flex justify-center items-center w-[60%]">
              {params.row.amount}
            </div>
          </>
        );
      },
    },
    {
      field: "bank_details",
      headerName: "Bank Details",
      flex: 0.5,
      minWidth: 100,

      renderCell: (params) => <BankDetailsCell data={params.row} />,
    },
    {
      field: "requestDate",
      headerName: "Request Date",
      flex: 0.5,
      minWidth: 100,
      valueFormatter: (pa) => formatDate(pa.value),
    },
    { field: "status", headerName: "Status", flex: 0.3, minWidth: 80 },
    {
      field: "  ",
      headerName: "Actions",
      flex: 0.3,
      minWidth: 80,
      renderCell: (params) => <TeacherInvoiceActions data={params.row} />,
    },
  ];

  const { filterData: rows, searchProps } =
    useSearchBar<PaymentInvoiceType>(data);

  return (
    <div className="flex flex-col gap-4 w-full p-3 rounded-[14px] shadow-xl border border-bcolor">
      <div className="w-full flex bg-[#ebebeb] py-[15px] px-[10px] rounded-[10px] justify-between items-center gap-5">
        <div className="mb-[8px]">
          <SearchBar {...searchProps} />
        </div>
        {/* add button here */}
      </div>
      <div className="w-full">
        <Box>
          <Box
            height="90vh"
            className="rounded-md"
            sx={{
              "& .MuiDataGrid-root": {
                border: "none",
                outline: "none",
              },
              "& .css-pqjvzy-MuiSvgIcon-root-MuiSelect-icon": {
                color: "#2a3547",
              },
              "& .MuiDataGrid-sortIcon": {
                color: "#2a3547",
              },
              "& .MuiDataGrid-row": {
                color: "#2a3547",
                borderBottom: "1px solid #2a3547fff30!important",
              },
              "& .MuiTablePagination-root": {
                color: "#2a3547",
              },
              "& .MuiDataGrid-cell": {
                borderBottom: "none!important",
              },
              "& .name-column--cell": {
                color: "#2a3547",
              },
              "& .MuiDataGrid-columnHeaders": {
                backgroundColor: "#F4F4F5",
                borderBottom: "none",
                color: "#2a3547",
              },
              "& .MuiDataGrid-virtualScroller": {
                backgroundColor: "#fff",
              },
              "& .MuiDataGrid-footerContainer": {
                color: "dark",
                borderTop: "none",
                backgroundColor: "#F4F4F5",
              },
              "& .MuiCheckbox-root": {
                color: `#b7ebde !important`,
              },
              "& .MuiDataGrid-toolbarContainer .MuiButton-text": {
                color: `#2a3547 !important`,
              },
              "& .css-ptkaw2-MuiDataGrid-root .MuiDataGrid-row.Mui-selected": {
                backgroundColor: `#e0e0e0`,
              },
              "& .css-ptkaw2-MuiDataGrid-root .MuiDataGrid-row.Mui-selected:hover":
                {
                  backgroundColor: `#F4F4F5`,
                },
            }}
          >
            <DataGrid rows={rows} columns={columns} />
          </Box>
        </Box>
      </div>
    </div>
  );
};

type BankDetailsCellProps = {
  data: PaymentInvoiceType;
};

function BankDetailsCell({ data }: BankDetailsCellProps) {
  const { isOpen, onOpen, onOpenChange } = useDisclosure();

  return (
    <>
      <div className="text-center flex justify-center items-center w-[60%]">
        <Button className="!bg-[transparent]" onPress={onOpen}>
          <FaRegFile className="!text-[#656565]" size={20} />
        </Button>
      </div>
      <Modal placement="center" isOpen={isOpen} onOpenChange={onOpenChange}>
        <BankDetailsModal bankDetails={data?.bankDetails} />
      </Modal>
    </>
  );
}

export default TeachersInvoice;
